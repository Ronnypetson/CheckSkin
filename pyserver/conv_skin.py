...
cl_type = 'cl'
#
model_ckpt = '/checkpoint/conv_skin/'
model_fn = model_ckpt+str(batch_size)+'_'+str(num_epochs*num_steps)+'_model.ckpt'
img_dir = '../'
#
img_dim = 50
learning_rate = 0.001
...
def get_test(test_directory=test_dir):
    ...
    for i in range(num_classes):
        ...
        for fl in fls:
            ...
            img = img/255.0
            #_,border = cv2.threshold(img,127,255,cv2.THRESH_BINARY)
            #X.append([img,border])
            X.append(img)
...
def get_batch():    # X: (batch_size,50,50), Y: (batch_size,2)
    X = []
    Y = []
    for i in range(batch_size):
        class_ = random.randint(0,num_classes-1)
        fl_loc = train_dir+classes[class_]
...
def vowel_fc(X):
      # Create the model
      x = tf.reshape(X,shape=[-1,num_classes*num_classes])
      W = tf.Variable(tf.zeros([num_classes*num_classes, num_classes]))
